	...
	return func(next gsf.HandlerFunc) gsf.HandlerFunc {
		return func(w gsf.ResponseWriter, r *gsf.Request) error {
			...
			attrs := []any{
				...
				slog.String("result_code_text", w.Header().ResultCodeText()),
				slog.Int("app_code", int(w.Header().AppCode)),
				slog.String("app_code_text", w.Header().AppCodeText()),
				...
				slog.String("svc_class_text", r.Header().ServiceClassText()),
				slog.Int("msg_type", int(r.Header().MsgType)),
				slog.String("msg_type_text", r.Header().MessageTypeText()),
