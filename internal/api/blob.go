...
func (h *Handler) PostBlobImport(w http.ResponseWriter, r *http.Request) error {
	...
	res.Message = fmt.Sprintf("Import completed: %d imported, %d skipped, %d metadata generated, %d metadata failed", result.ImportedFiles, result.SkippedFiles, result.GeneratedMetadata, result.FailedMetadata)
